...
{
  std::vector<ER_Patient> person_list{};
  ...
  {
    ER_Patient current_person = create_person_from_storage_record(current_record);
...
ER_Patient FD_DA_Patient_Repository_FileBasedImpl::find(unsigned int id)
{
  ...
  ER_Patient result = create_person_from_storage_record(record);
  return result;
};

ER_Patient FD_DA_Patient_Repository_FileBasedImpl::save(const ER_Patient &person)
{
  ...
  ER_Patient updated_person = create_person_from_storage_record(updated_record);
...
ER_Patient FD_DA_Patient_Repository_FileBasedImpl::create_person_from_storage_record(const std::string &line)
{
...
std::string FD_DA_Patient_Repository_FileBasedImpl::create_storage_record_from_person(unsigned int id, const ER_Patient &person)
{
  std::string result{};
