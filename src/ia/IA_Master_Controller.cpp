...
IA_Master_Controller::IA_Master_Controller(IA_Person_Controller& a_person_controller)
            : person_controller(a_person_controller){};

void IA_Master_Controller::control_application_start(){
    person_controller.control_list_all();
    control_menu_choice();
};
...
void IA_Master_Controller::control_menu_choice(){
    ...
    { 
        ...
        if (choice == '1')
        {
            person_controller.control_create();
            continue; 
        }
        if (choice == '2')
        {
            person_controller.control_update();
            continue; 
        }
        if (choice == '3')
        {
            person_controller.control_remove();
            continue; 
        }
        if (choice == '4')
        {
